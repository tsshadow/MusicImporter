...
async def execute_step(
    ...
    try:
        job["status"] = "running"
        await broadcast(job)
        await run_in_threadpool(step.run, {step_name}, **(options or {}))
        job["status"] = "done"
        while True:
...
async def run_step_endpoint(
    ...
    try:
        ...
        payload = {}
    options: Dict = {}
    if "breakOnExisting" in payload:
        options["break_on_existing"] = payload["breakOnExisting"]
    ...
    import asyncio
    asyncio.create_task(execute_step(step_name, job_id, options))
    return jobs[job_id]
